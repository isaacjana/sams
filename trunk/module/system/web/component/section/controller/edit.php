...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if(!\I::is_error()){
		...
		$v_result = \E::get('obj_system_module')->section_edit($v_update);
		unset($v_update);
		
		$v_lang_update['kwd_code_sec_id'] = $_POST['sec_id'];
		...
		foreach($v_lang_info as $v_lang){
			...
			$v_agency_id = \E::get('obj_system_module')->section_lbl_edit($v_lang_update);
		}
		unset($v_lang_update);
		header('location: ' . \E::get('obj_system_module')->build_action_path('section','list'));
	}
	$v_var = array(
				'ag_name' => $v_sec_info['ag_name'],
				'ad_name' => $v_sec_info['ad_name']);
	foreach($_POST as $v_key => $v_value){
		$v_var[$v_key] = trim($v_value);
	}
}else{
	$v_var = array(
				'sec_id' => $v_sec_info['sec_id'],
				'ag_name' => $v_sec_info['ag_name'],
				'ad_name' => $v_sec_info['ad_name'],
				'ad_name' => $v_sec_info['ad_name'],
				'sec_name' => $v_sec_info['kwd_value'],
				'sec_short_name' => $v_sec_info['kwd_short_name']
				);
}
