...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	foreach($v_lang_info as $v_lang){
		if(!\I::check("COMPULSORY", 'ad_name_'.$v_lang, \E::get("obj_kernel_lang")->get('ERR_JS_FIELD_REQUIRED'))){
			...
			$v_cond['sec_id@NOT_IN'] = $_POST['sec_id'];
	...
	if(!\I::is_error()){
		...
		$v_ad_id = \E::get('obj_system_module')->section_edit($v_update);
		unset($v_update);
...
//===ELSE: Get the section information from database.
	unset($v_cond);
	$v_cond['sec_id'] = $v_param[0];
	...
	foreach($v_lang_info as $v_lang){
		...
		if(is_array($v_result)){
			$v_sec_info['lang'][$v_lang]['kwd_value'] = $v_result['kwd_value'];
			$v_sec_info['lang'][$v_lang]['kwd_short_name'] = $v_result['kwd_short_name'];
		}
	}
	
	$v_var['ag_id'] = $v_result['ag_id'];
	$v_var['ad_id'] = $v_result['ad_id'];
	$v_var['sec_id'] = $v_param[0];
}
