...
$v_param['ad_id'] = ( !isset($_GET['ad_id']) || $_GET['ad_id'] == '' ) ? '' : $_GET['ad_id'];
...
$v_access = false;
$v_access_num = 0;
$v_system_agency = \E::get_session(\E::get('obj_system_module')->get_codename().'/system_profile/group_adm_agency_sec');
...
if(is_array($v_arr_agency)){
	foreach($v_arr_agency as $v_key => $v_value){
		...
		if( $v_temp[0] == $v_param['ag_id'] && 
				($v_temp[1] == $v_param['ad_id'] || $v_temp[1] == 'hq' )&& 
				\U::is_user_able_to('READ', $v_value['group_adm_agency_sec'])){
			$v_access = true;
			$v_access_num = ($v_access_num < $v_value['group_adm_agency_sec'])? $v_value['group_adm_agency_sec'] : $v_access_num;
			break;
		}
	}
}
...
if($v_access){
	...
	$v_param['toggle'] .= '&ad_id='.urlencode($v_param['ad_id']);
	...
	$v_param['paging'] .= '&ad_id='.urlencode($v_param['ad_id']);
	...
	if(!$v_agency_read){
		...
		$v_total_rec = \E::get( 'obj_system_module')->section_list_total($v_cond);
	}
