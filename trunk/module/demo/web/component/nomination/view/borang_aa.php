		...
		<div class="col-12 col-md-6">  
			<div class="col-12">
				...
				<div class="form-group row">
					...
					<div class="col-12 d-flex py-2">
						<div class="form-check col-6 row mx-3">
							<input class="form-check-input" type="radio" name="jantina" id="lelaki" <?php echo \U::safe_display_string(($v_bahagian_a_data['nom_gender']=='l') ? "checked" : "");?> value="lelaki">
							<label class="form-check-label" for="lelaki">Lelaki</label>
						</div>
						<div class="form-check col-6 row mx-0 px-0 mx-3">
							<input class="form-check-input" type="radio" name="jantina" id="perempuan" <?php echo \U::safe_display_string(($v_bahagian_a_data['nom_gender']=='p') ? "checked" : "");?> value="perempuan">
				...
				<div class="form-group row">
					...
					<?php
								$v_race_id=\U::safe_display_string(isset($v_bahagian_a_data['race_id']) ? $v_bahagian_a_data['race_id'] : "");
                                $v_opt = '';
                                $v_count = 1;
                                if ($v_row["race_id"]=='') {
                                ...
                                foreach($v_keturunan as $v_row){
                                	$select='';
                                	if ($v_race_id==$v_row["race_id"]) {
                                		$select='selected';
                                	}

                                    if($v_count==1){
                                        $v_opt .= '<option value="'.\U::safe_display_string($v_row["race_id"]).'" '.$select.'>'.\U::safe_display_string($v_row["race_desc"]).'</option>';    
                                    }else{
                                        $v_opt .= '<option value="'.\U::safe_display_string($v_row["race_id"]).'" '.$select.'>'.\U::safe_display_string($v_row["race_desc"]).'</option>';
                                    }
                                    ++$v_count;
		...
		<div class="col-12 col-md-6">
			<div class="col-12">
				...
				<div class="form-group row">
					...
					<input class="w-75 form-control" type="text" id="poskod" name="poskod" value="<?php echo \U::safe_display_string(isset($v_bahagian_a_data['nom_postcode']) ? $v_bahagian_a_data['nom_postcode'] : "");?>">
				</div>
				<div class="form-group row">
					...
					<?php
                                $v_opt = '';
                                $v_count = 1;
                                if (!isset($v_bahagian_a_data['city_id'])) {
                                	$v_opt .= '<option value="" selected></option>';
                                }
                                
                                foreach($v_bandar as $v_row){
                                    if($v_count==1){
                                        $v_opt .= '<option  value="'.\U::safe_display_string($v_row["city_id"]).'" '.\U::safe_display_string(($v_bahagian_a_data['city_id']==$v_row["city_id"]) ? "selected" : "").'>'.\U::safe_display_string($v_row["city_name"]).'</option>';    
                                    }else{
                                        $v_opt .= '<option value="'.\U::safe_display_string($v_row["city_id"]).'" '.\U::safe_display_string(($v_bahagian_a_data['city_id']==$v_row["city_id"]) ? "selected" : "").'>'.\U::safe_display_string($v_row["city_name"]).'</option>';
                                    }
                                    ++$v_count;
				...
				<div class="form-group row">
					...
					<?php
                                ...
                                if (!isset($v_bahagian_a_data['division_id'])) {
                                	$v_opt .= '<option value="" selected></option>';
                                }
                                foreach($v_bahagian as $v_row){
                                    if($v_count==1){
                                        $v_opt .= '<option value="'.\U::safe_display_string($v_row["division_id"]).'" '.\U::safe_display_string(($v_bahagian_a_data['division_id']==$v_row["division_id"]) ? "selected" : "").'>'.\U::safe_display_string($v_row["division_name"]).'</option>';    
                                    }else{
                                        $v_opt .= '<option value="'.\U::safe_display_string($v_row["division_id"]).'" '.\U::safe_display_string(($v_bahagian_a_data['division_id']==$v_row["division_id"]) ? "selected" : "").'>'.\U::safe_display_string($v_row["division_name"]).'</option>';
                                    }
                                    ++$v_count;
				...
				<div class="form-group row">
					...
					<?php
                                $v_opt = '';
                                $v_count = 1;
                                if (!isset($v_bahagian_a_data['state_id'])) {
                                	$v_opt .= '<option value="" selected></option>';
                                }
                                foreach($v_negeri as $v_row){
                                    if($v_count==1){
                                        $v_opt .= '<option value="'.\U::safe_display_string($v_row["state_id"]).'" '.\U::safe_display_string(($v_bahagian_a_data['state_id']==$v_row["state_id"]) ? "selected" : "").'>'.\U::safe_display_string($v_row["state_name"]).'</option>';    
                                    }else{
                                        $v_opt .= '<option value="'.\U::safe_display_string($v_row["state_id"]).'" '.\U::safe_display_string(($v_bahagian_a_data['state_id']==$v_row["state_id"]) ? "selected" : "").'>'.\U::safe_display_string($v_row["state_name"]).'</option>';
                                    }
                                    ++$v_count;
				...
				<div class="form-group row">
					...
					<input class="w-75 form-control" type="text" id="telefon_rumah" name="telefon_rumah" value="<?php echo \U::safe_display_string(isset($v_bahagian_a_data['nom_contact']) ? $v_bahagian_a_data['nom_contact'] : "");?>">
				</div>
				<div class="form-group row">
					<label class="w-25 col-form-label" for="telefon_bimbit">Nombor Telefon Bimbit:</label>
					<input class="w-75 form-control" type="text" id="telefon_bimbit" name="telefon_bimbit" value="<?php echo \U::safe_display_string(isset($v_bahagian_a_data['nom_mobile']) ? $v_bahagian_a_data['nom_mobile'] : "");?>">
				</div>
				<div class="form-group row">
